...
try{
 ...
 await fill('textarea[placeholder="y_true,y_pred,y_probability"]','y_true,y_pred,y_probability\n0,0,0.1');await fill('[aria-describedby=training-log-help]','epoch=1 train_loss=0.8');await fill('.astra-inputs input[maxlength="4000"]','Restore this local draft');await evaluate("(()=>{const e=document.querySelector('.astra-inputs select');e.value='shift';e.dispatchEvent(new Event('change',{bubbles:true}));})()");await click('#save-investigation-draft');await waitFor(()=>evaluate("Boolean(localStorage.getItem('whylab-astra-draft-v1'))"),'draft saved');await command('Page.reload');await waitFor(()=>evaluate("Boolean(document.querySelector('[aria-describedby=training-log-help]')?.value?.includes('train_loss=0.8'))"),'draft restored');assert.equal(await evaluate("document.querySelector('.astra-inputs input[maxlength=\"4000\"]').value"),'Restore this local draft');assert.equal(await evaluate("document.querySelector('.astra-inputs select').value"),'shift');assert.equal(await evaluate("document.querySelector('.astra-consent input').checked"),false);assert.equal(await evaluate("document.querySelector('[aria-describedby=access-token-help]').value"),'');await button('Discard saved draft');assert.equal(await evaluate("localStorage.getItem('whylab-astra-draft-v1')"),null);pass('Explicit local draft recovery excludes consent and access token');
 await button('Load synthetic training logs');assert.ok(await evaluate("document.querySelector('[aria-describedby=training-log-help]').value.includes('epoch=3')"));await waitFor(()=>evaluate("document.querySelector('[aria-label=\"Training log diagnostics\"]')?.textContent.includes('Possible overfitting')"),'training-log preview');assert.ok(await evaluate("document.querySelector('[aria-label=\"Training log diagnostics\"]').textContent.includes('unverified')"));await button('Clear evidence');assert.equal(await evaluate("document.querySelector('[aria-describedby=training-log-help]').value"),'');assert.equal(await evaluate("Boolean(document.querySelector('[aria-label=\"Training log diagnostics\"]'))"),false);pass('Structured training-log diagnostics and clear controls');
 ...
 await evaluate(`window.testHypothesis=[...document.querySelectorAll('.diagnosis-item')].find(e=>e.querySelector('summary').textContent.includes('overfitting'));testHypothesis.open=true;testHypothesis.querySelector('.interactive-lesson').open=true;testHypothesis.scrollIntoView({behavior:'instant',block:'start'});`);await screenshot('hypothesis_desktop_overview.png');
 ...
 await evaluate("window.testHypothesis.querySelector('.interactive-lesson').scrollIntoView({behavior:'instant',block:'start'});");await screenshot('hypothesis_interactive_lesson.png');
 ...
 await evaluate(`testHypothesis.querySelector('.experiment-workflow').open=true;testHypothesis.querySelector('.experiment-workflow').id='test-experiment';window.testHypothesis.querySelector('.experiment-workflow').scrollIntoView({behavior:'instant',block:'start'});`);await screenshot('hypothesis_verification_form.png');
 ...
 await screenshot('hypothesis_verification_recorded.png');
 await command('Emulation.setDeviceMetricsOverride',{width:375,height:812,deviceScaleFactor:2,mobile:true});await pause(200);await evaluate("window.testHypothesis.scrollIntoView({behavior:'instant',block:'start'});");await screenshot('hypothesis_mobile_responsive.png');await command('Emulation.setDeviceMetricsOverride',{width:1440,height:1000,deviceScaleFactor:1,mobile:false});await pause(200);
