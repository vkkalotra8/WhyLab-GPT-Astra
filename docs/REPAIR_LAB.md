...
The homepage Repair Lab accepts evaluation CSV text and an explicit objective, false-negative cost, false-positive cost, maximum total cost, and baseline threshold. **Load repair example** loads the committed synthetic melanoma fixture. Labels are currently 1/0, with probability referring to label 1. CSV size is limited to 2 MB and existing ingestion row/column limits apply. Numeric cost fields are authoritative; the application does not infer a numeric policy from prose.

## Workflow

1. Select **Measure local candidates**, or confirm consent and select **Ask Astra for repair**.
2. Inspect all operating-point metrics and candidate threshold. The current policy remains the baseline.
3. Select **Apply recommended policy** to run the existing re-evaluation engine on an evaluation copy. Live metrics and confusion counts change only after application. Re-test status and the canonical evidence graph appear.
4. **Restore baseline preview** returns the display to the original baseline. This discards the current applied preview; it is not a durable repair history.
...
`tests/repair-lab.test.mjs` covers input policy bounds, candidate/application separation, changed costs, infeasible targets, canonical immutability, two-turn provider execution, tool refusal/validation, false directions, transport failures, cancellation, consent, origin and missing configuration. Provider tests use mocked fetch, not a production fallback. The browser suite covers local example preparation, explicit application, baseline restore and stale-result invalidation after policy changes. A live account/model call remains a manual configuration check.
