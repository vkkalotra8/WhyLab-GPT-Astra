...
import { prepareRepair, measuredTradeoffs, validateRepairLabInput } from '../investigation/repair-lab.ts';
...
export async function recommendRepairWithAstra(value: unknown, signal: AbortSignal, contextValue?: unknown) {
  ...
  for (const item of second.output) { if (item.type === 'function_call') throw new Error('Unexpected tool call.'); if (item.type === 'message') for (const part of item.content ?? []) { if (part.type === 'refusal') throw new Error('Provider declined interpretation.'); if (part.type === 'output_text') fragments.push(part.text); } }
  const explanation = JSON.parse(fragments.join(''));
