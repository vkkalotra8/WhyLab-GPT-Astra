...
const compositeCriterion = object({ kind: choice('all_of'), criteria: list(diagnosticCriterion, 4, 2) });
const falsificationCriterion: JsonSchema = { anyOf: [diagnosticCriterion, compositeCriterion] };
const definitions = {
  ...
  evaluate_diagnostic_falsification: [object({ hypothesisId:string,resultId:string,prediction:string,criterion:falsificationCriterion }), 'Evaluate a registered non-accuracy hypothesis against measured metrics from one completed diagnostic result. Declare the prediction and threshold before evaluation. Pass a single criterion, or {kind:"all_of",criteria:[...]} with 2-4 distinct metrics for a conjunctive prediction: all criteria met supports it, some met weakens it, none met rejects it. Ambiguous, absent or undefined metrics make the whole prediction inconclusive.'],
